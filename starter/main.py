...
def get_model():
    print (os.listdir())
    return joblib.load(f"{folder_path}/model/model.joblib"), joblib.load(f"{folder_path}/model/encoder.joblib"),joblib.load(f"{folder_path}/model/lb.joblib")
...
async def predict_salary(item: InputItem):
    ...
    data = process_data(df, encoder = encoder, lb = lb, label = None, categorical_features = cat_features, training = False)[0]
